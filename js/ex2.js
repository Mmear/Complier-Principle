...
* /
    ...
    let operationPredict = function() {
        ...
        let getFirst = function(ch) {
            ...
            for(let j = 0; j < product.length; j++) {
                ...
                if(TERMINAL.indexOf(first_ch) !== -1 || first_ch === 'ε') {
                    //产生式首字符为终结符或空字
                    ...
                    for(; k < product[j].length && hasNull !== -1; k++) {
                        ...
                        hasNull = result.indexOf('ε');
                    }
                    if(k === product[j].length && hasNull !== -1){
                        //所有非终结符均含有空字
                        temp_table.uniquePush('ε');
                    }
                ...
                if(grammar.hasOwnProperty(left)) {
                    ...
                    for(let i = 0; i < products.length; i++) {
                        ...
                        if(product[len-2] === ch) {
                            ...
                            let nullIndex = first_b.indexOf('ε'); //检查是否为空
                            if(nullIndex !== -1){
                                if(left !== ch){
                                    //将产生式左部FOLLOW集加入，如果左部是本身，跳过
                                    temp_table.uniquePush(getFollow(left));
    ...
    /*主控程序*/
    let main_control_predict = function(str) {
